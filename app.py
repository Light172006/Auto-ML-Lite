...
if st.text_input('Enter The Target',key = 'target'):
    ...
    if st.session_state.target not in features_names:
        ...
        data = classification.clean(data,st.session_state.target)
        X_axis = data.drop([st.session_state.target],axis=1)
        y_axis = data[st.session_state.target]
        X_axis = classification.feature_engineering(X_axis)
        X_axis = classification.X_encoding(X_axis)
        X_axis = classification.dimention_reduction(X_axis)
        y_axis = classification.y_encoding(y_axis)


if st.button('Train'):
    st.warning('Your Model Is Training')
    st.spinner("Training model...")
    best_model,best_score,  = classification.train(X_axis,y_axis)
